...
export async function createAuctionManager(
  ...
  const lookup = {
    ...
    startAuction: await setupStartAuction(
      wallet,
      vault,
      storePubKey,
      auctionManager
    ...
    validateBoxes: await validateBoxes(
      ...
      safetyDepositConfigs.filter(
        (c) =>
          ...
          (participationSafetyDepositDraft.masterEdition?.data.key ==
  ...
  while (stopPoint < instructions.length && tries < 3) {
    ...
    try {
      if (instructions.length === 1) {
        ...
        const txResponse = await sendTransactions(
          connection,
          wallet,
          instructions,
          filteredSigners,
        ...
        stopPoint = txResponse.failedAt;

        if (txResponse.lastTxId) {
          lastTxId = txResponse.lastTxId;
        }
      }
    } catch (e) {
      console.error(e);
  ...
  return { lastTxId, vault, auction, auctionManager };
}
      ...
      box: {
        ...
        tokenMint:
          s.winningConfigType !== WinningConfigType.PrintingV1
            ...
            : (s.masterEdition?.data as MasterEditionV1Data).printingMint,
        amount:
